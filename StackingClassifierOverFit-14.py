...
lr_2 = LogisticRegression(C = 1.5, max_iter = 2000, class_weight = 'balanced', penalty = 'l2', solver = 'sag')
lr_3 = LogisticRegression(C = 0.5, max_iter = 2000, class_weight = 'balanced', penalty = 'l2', solver = 'liblinear')
lr_4 = LogisticRegression(C = 1, max_iter = 2000, class_weight = 'balanced', penalty = 'l2', solver = 'liblinear')
...
np.random.seed(0)
sclf = StackingCVClassifier(classifiers = [lgbm_1, xgb_1,rf_1, knn_1,gnb_1, sgd_1, svc_1, lr_1], 
                            use_probas = True,
...
test_result_non_transform = np.zeros(len(test)) ; auc_score_non_transform = 0 ; cond_non_transform = 0
test_result_noise = np.zeros(len(test)) ; auc_score_noise = 0 ; cond_noise = 0
test_result_scale = np.zeros(len(test)) ; auc_score_scale = 0 ; cond_scale = 0
test_result_robust = np.zeros(len(test)) ; auc_score_robust = 0 ; cond_robust = 0
test_result_noise_2 = np.zeros(len(test)) ; auc_score_noise_2 = 0 ; cond_noise_2 = 0
test_result_noise_3 = np.zeros(len(test)) ; auc_score_noise_3 = 0 ; cond_noise_3 = 0
test_result_noise_4 = np.zeros(len(test)) ; auc_score_noise_4 = 0 ; cond_noise_4 = 0
...
print((auc_score_robust + auc_score_scale + auc_score_noise + auc_score_noise_2 +
      auc_score_noise_3 + auc_score_noise_4 + auc_score_non_transform)/7)

ens_DF_AllModels = pd.DataFrame({'Noise' : test_result_noise,
                       'Noise2' : test_result_noise_2,
                       'Noise3' : test_result_noise_3,
                       'Noise4' : test_result_noise_4,
                       'Robust' : test_result_robust,
                       'Scale': test_result_scale,
                       'Non-Trans': test_result_non_transform})
    
ens_DF_mean_AllModels = pd.DataFrame((test_result_noise +
                            test_result_noise_2 +
                            test_result_noise_3 +
                            test_result_noise_4 + 
                            test_result_robust +
                            test_result_scale + 
                            test_result_non_transform) / 7)
