...
test_result_noise_5 = np.zeros(len(test))
auc_score_noise_5 = 0
cond_noise_5 = 0

test_result_noise_6 = np.zeros(len(test))
auc_score_noise_6 = 0
cond_noise_6 = 0
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(train, targets)):
    ...
    ##noise_5
    np.random.seed(10)
    noise_5 = np.random.normal(0.15, 0.15, train.shape)
    train_noise_5 = train + noise_5
    X_train, y_train = train_noise_5.iloc[trn_idx], targets.iloc[trn_idx]
    X_valid, y_valid = train_noise_5.iloc[val_idx], targets.iloc[val_idx]
    sclf.fit(X_train.values, y_train.values)
    y_pred = sclf.predict_proba(X_valid.values)
    auc = roc_auc_score(y_valid, y_pred[:, 1])
    print('Fold: ', fold_ + 1, 'Noise_5:', auc)
    auc_score_noise_5 += auc
    preds_noise_5 = sclf.predict_proba(test.values)
    test_result_noise_5 += preds_noise_5[:, 1]
    cond_noise_5 += 1
    ##noise_6
    np.random.seed(10)
    noise_6 = np.random.normal(0.02, 0.02, train.shape)
    train_noise_6 = train + noise_6
    X_train, y_train = train_noise_6.iloc[trn_idx], targets.iloc[trn_idx]
    X_valid, y_valid = train_noise_6.iloc[val_idx], targets.iloc[val_idx]
    sclf.fit(X_train.values, y_train.values)
    y_pred = sclf.predict_proba(X_valid.values)
    auc = roc_auc_score(y_valid, y_pred[:, 1])
    print('Fold: ', fold_ + 1, 'Noise_6:', auc)
    auc_score_noise_6 += auc
    preds_noise_6 = sclf.predict_proba(test.values)
    test_result_noise_6 += preds_noise_6[:, 1]
    cond_noise_6 += 1
...
auc_score_noise_5 = auc_score_noise_5 / cond_noise_5
print("AUC score_noise_5: ", auc_score_noise_5)
test_result_noise_5 = test_result_noise_4 / cond_noise_5

auc_score_noise_6 = auc_score_noise_6 / cond_noise_6
print("AUC score_noise_6: ", auc_score_noise_6)
test_result_noise_6 = test_result_noise_6 / cond_noise_6
