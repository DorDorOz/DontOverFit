...
meta_lr = LogisticRegression(C = 1, max_iter = 2000, class_weight = 'balanced', penalty = 'l2', solver = 'sag')
meta_rf = RandomForestClassifier(n_estimators = 100, max_depth = 4, random_state = 0)
...
np.random.seed(0)
sclf = StackingCVClassifier(classifiers = [lr_1, lr_2, lr_3, lr_4, lr_5, lr_6, lr_7,
...
print("All-AUC", (auc_score_MinMax_PCA + 
                  auc_score_Robust_PCA + 
                  auc_score_std_Scaled_PCA) / 3)

final_Ens = pd.DataFrame((test_result_std_Scaled_PCA + 
                          test_result_Robust_PCA + 
                          test_result_MinMax_PCA) / 3)


model_1_ens_lr = pd.concat([pd.DataFrame(testSub_Ids), pd.DataFrame(final_Ens)], axis = 1)
model_1_ens_lr.columns = ['id', 'target']
model_1_ens_lr.to_csv(r'D:/Python/data/DontOverFit/PCAStacking.csv', index = False)
